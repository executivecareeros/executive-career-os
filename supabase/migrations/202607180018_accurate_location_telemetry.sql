...
with authorized as (
  ...
  select count(distinct aliases.code)::int as countries_represented,
    count(distinct lower(nullif(opportunity.country,''))) filter(
      where aliases.code is null
        and lower(coalesce(opportunity.country,'')) not in ('','not specified','unknown','remote')
    )::int as unrecognized_location_labels
...
select case when(select allowed from authorized) then jsonb_build_object(
  'version','operational-coverage-summary-v3','measuredAt',observed_at,
...
comment on function public.get_operational_coverage_summary(uuid,timestamptz) is 'Bounded scheduler telemetry with deterministic ISO country recognition; deliberate unknown sentinels are excluded from invalid-label counts.';
