...
begin
  ...
  with scoped as (
    ...
    select distinct on(executive_identity_id) executive_identity_id,role_title
    from public.professional_experiences
    where archived_at is null and coalesce(trim(role_title),'')<>''
      and exists(select 1 from scoped s where s.executive_identity_id=professional_experiences.executive_identity_id)
    ...
    select jsonb_build_object(
      ...
      'currentTitles',coalesce((select jsonb_agg(jsonb_build_object('name',role_title,'executives',executives) order by executives desc,role_title) from (
        select role_title,count(*) executives from current_titles group by role_title
      ) q),'[]'::jsonb),
  ...
  select jsonb_build_object(
    ...
    'registeredExecutives',summary.registered_executives,'activeNow',summary.active_now,
...
comment on function public.get_founder_product_learning_dashboard(integer) is 'Founder-only aggregate executive analytics with registered, active-now, retention, engagement, device and voluntarily confirmed professional dimensions; raw activity and identities are never returned.';
