...
begin
  ...
  for employer in
    with greenhouse as (
      select o.id,
        ...
        coalesce(
          lower((regexp_match(source->>'originalUrl','greenhouse\.io/([^/]+)/jobs'))[1]),
          case when source->>'originalId' ~ '^[a-zA-Z0-9_-]+-[0-9]+$'
            then lower(regexp_replace(source->>'originalId','-[0-9]+$',''))
          end
        ) board_token,
  ...
  loop
    company_uuid := public.upsert_employer_observation(
      ...
      jsonb_build_object('sourceKind','Employer','evidenceStatus','Partial','backfillVersion','greenhouse-employer-v2')
    );
  ...
  return jsonb_build_object('version','greenhouse-employer-backfill-v2','employersResolved',employer_count,'opportunitiesLinked',linked_count,'observedAt',observed_at);
end
$$;

comment on function public.backfill_greenhouse_employers(uuid,uuid,timestamptz) is
  'Idempotently resolves existing workspace Greenhouse observations using explicit board URL or validated Greenhouse source identity provenance.';
