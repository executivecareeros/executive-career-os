...
as $$
  select opportunity.id, opportunity.domain_id, opportunity.company_id, opportunity.version,
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'summary',to_jsonb(left(coalesce(opportunity.payload->>'summary',''),2000)),
...
comment on function public.get_opportunity_candidates(uuid,integer) is
  'Workspace-authorized compact candidate feed for executive search and deterministic ranking. Full evidence remains on the canonical opportunity record.';
