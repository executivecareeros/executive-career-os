...
type Extraction = { filename: string; format: string; drafts: HistoryDocumentDraft[]; warnings: string[] };
...
export function ImportWorkspace({ locale = "en" }: { locale?: Locale }) {
  ...
  async function extract(file?: File) {
    if (!file) return;
    setBusy(true); setError(undefined); setExtraction(undefined); setDrafts([]);
    try {
      ...
      setExtraction(result); setDrafts(result.drafts);
  ...
  return <main className="mx-auto max-w-5xl px-5 py-8 text-[#17191c] sm:px-8">
    <p className="text-xs font-semibold uppercase tracking-[.18em] text-[#6f8796]">{tr ? "CV incelemesi" : "CV review"}</p>
    ...
    <section className="mt-8 rounded-2xl border border-[#e3e5e6] bg-white p-5 shadow-sm sm:p-7">
      <label className="block cursor-pointer rounded-2xl border border-dashed border-[#cdd2d5] bg-[#f8f9f9] p-8 text-center focus-within:ring-2 focus-within:ring-[#8ea7b8]">
    ...
    {extraction && <form action={confirmCvHistory} className="mt-7">
      ...
      <input type="hidden" name="drafts" value={JSON.stringify(drafts.map(({ organizationName, roleTitle, startDate, endDate, isCurrent }) => ({ organizationName, roleTitle, startDate, endDate, isCurrent })))}/>
      <section className="rounded-2xl border border-[#e3e5e6] bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-[#536f80]">{extraction.filename} · {extraction.format}</p><h2 className="mt-2 text-xl font-semibold">{tr ? "Atlas'ın bulduğu deneyimi doğrula" : "Confirm the experience Atlas found"}</h2><p className="mt-2 text-sm leading-6 text-[#6f757b]">{tr ? "Yalnızca doğru olduğunu bildiğin bilgileri onayla. Her alanı şimdi doldurman gerekmez." : "Confirm only what you know is correct. You do not need to complete every detail now."}</p></div><span className="rounded-full bg-[#edf3f5] px-3 py-1 text-xs font-medium text-[#36576a]">{drafts.length} {tr ? "rol" : drafts.length === 1 ? "role" : "roles"}</span></div>
        {drafts.length ? <div className="mt-6 space-y-4">{drafts.map((draft, index) => <article key={draft.id} className="rounded-2xl border border-[#e5e7e8] bg-[#fafafa] p-5"><div className="grid gap-4 sm:grid-cols-2"><Field label={tr ? "Şirket" : "Employer"} value={draft.organizationName} onChange={value => update(draft.id, { organizationName: value })}/><Field label={tr ? "Rol" : "Role"} value={draft.roleTitle} onChange={value => update(draft.id, { roleTitle: value })}/><Field label={tr ? "Başlangıç" : "Start"} type="month" value={draft.startDate ?? ""} onChange={value => update(draft.id, { startDate: value || undefined })}/><Field label={tr ? "Bitiş" : "End"} type="month" value={draft.endDate ?? ""} disabled={draft.isCurrent} onChange={value => update(draft.id, { endDate: value || undefined })}/></div><div className="mt-4 flex items-center justify-between gap-4"><label className="flex items-center gap-2 text-sm text-[#565c62]"><input type="checkbox" checked={draft.isCurrent} onChange={event => update(draft.id, { isCurrent: event.target.checked, endDate: event.target.checked ? undefined : draft.endDate })}/>{tr ? "Halen bu roldeyim" : "I currently hold this role"}</label><button type="button" onClick={() => setDrafts(items => items.filter(item => item.id !== draft.id))} className="text-sm text-[#7a4d4d]">{tr ? "Kaldır" : "Remove"} {index + 1}</button></div></article>)}</div> : <p className="mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">{tr ? "CV'den güvenilir bir rol çıkarılamadı. Aşağıdan temel rolünü ekleyebilirsin; iş aramaya yine devam edebilirsin." : "No role could be extracted reliably. Add an essential role below, or continue searching jobs without it."}</p>}
