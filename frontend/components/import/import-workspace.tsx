...
export function ImportWorkspace({ locale = "en" }: { locale?: Locale }) {
  ...
  async function extract(file?: File) {
    if (!file) return;
    setBusy(true); setError(undefined); setExtraction(undefined); setDrafts([]); setProfile({}); setHighlights([]); setEducation([]); setLanguages([]); setSkills([]); setCertifications([]); setCustomSections([]);
    try {
      ...
      setExtraction(result); setDrafts(result.drafts); setProfile(result.profile ?? {}); setHighlights(result.highlights ?? []); setEducation(result.education ?? []); setLanguages(result.languages ?? []); setSkills(result.skills ?? []); setCertifications(result.certifications ?? []); setCustomSections([]);
  ...
  return <main className="mx-auto max-w-5xl px-5 py-8 text-[#17191c] sm:px-8">
    ...
    {extraction && <form action={confirmCvHistory} className="mt-7">
      ...
      <input type="hidden" name="drafts" value={JSON.stringify(drafts)}/><input type="hidden" name="documentContext" value={JSON.stringify({ profile, highlights, education, languages, skills, certifications, customSections })}/>
      <section className="rounded-2xl border border-[#e3e5e6] bg-white p-5 shadow-sm sm:p-7">
        ...
        <ProfileReview tr={tr} profile={profile} setProfile={setProfile} highlights={highlights} setHighlights={setHighlights} education={education} setEducation={setEducation} languages={languages} setLanguages={setLanguages} skills={skills} setSkills={setSkills} certifications={certifications} setCertifications={setCertifications} customSections={customSections} setCustomSections={setCustomSections}/>
...
function RoleReviewCard({ draft, index, tr, update, remove }: { draft: HistoryDocumentDraft; index: number; tr: boolean; update: (id: string, change: Partial<HistoryDocumentDraft>) => void; remove: () => void }) {
  return <article className="rounded-2xl border border-[#e5e7e8] bg-[#fafafa] p-5">
    <div className="grid gap-4 sm:grid-cols-2">
      <Field required label={tr ? "Şirket" : "Company"} value={draft.organizationName} onChange={value => update(draft.id, { organizationName: value })}/>
      <Field required label={tr ? "Rol" : "Role"} value={draft.roleTitle} onChange={value => update(draft.id, { roleTitle: value })}/>
      ...
      <Field label={tr ? "Konum" : "Location"} value={draft.location ?? ""} onChange={value => update(draft.id, { location: value || undefined })}/>
      <Field label={tr ? "Çalışma türü" : "Employment type"} value={draft.employmentType ?? ""} onChange={value => update(draft.id, { employmentType: value || undefined })}/>
    </div>
    <div className="mt-4 grid gap-4">
      ...
      <TextArea label={tr ? "Liderlik kapsamı" : "Leadership scope"} value={draft.leadershipScope ?? ""} onChange={value => update(draft.id, { leadershipScope: value || undefined })}/>
      <TextArea label={tr ? "Coğrafi sorumluluk" : "Geographic responsibility"} value={draft.geographicResponsibility ?? ""} onChange={value => update(draft.id, { geographicResponsibility: value || undefined })}/>
      <TextArea label={tr ? "Sektörler (satır başına bir tane)" : "Industries (one per line)"} value={(draft.industries ?? []).join("\n")} onChange={value => update(draft.id, { industries: splitLines(value) })}/>
      <TextArea label={tr ? "Teknolojiler (satır başına bir tane)" : "Technologies (one per line)"} value={(draft.technologies ?? []).join("\n")} onChange={value => update(draft.id, { technologies: splitLines(value) })}/>
    </div>
