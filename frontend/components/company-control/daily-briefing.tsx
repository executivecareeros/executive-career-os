  ...
  return (
    <SectionCard>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        {brief.sections.map((section) => <article key={section.id} className="min-w-0 rounded-xl border border-white/10 bg-slate-950/35 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-medium text-white">{section.title}</h3><CompanyStatus status={section.status}/></div><p className="mt-3 text-sm leading-6 text-slate-400">{section.summary}</p><p className="mt-3 break-all text-xs text-slate-600">Evidence: {section.evidence[0]?.sourceReference}</p></article>)}
      </div>
