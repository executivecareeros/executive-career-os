...
export function TopBar({ onOpenMenu, menuOpen, locale = "en", signedInEmail }: TopBarProps) {
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#e5e7e8] bg-white/95 px-5 text-[#17191c] backdrop-blur lg:hidden">
      <div className="flex min-w-0 items-center gap-3"><Link href="/" aria-label="ORENDALIS Home" className="rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-[#936b3f]"><OrendalisMark /></Link>{signedInEmail && <span className="max-w-[42vw] truncate rounded-full bg-[#f3eee7] px-3 py-1 text-xs font-medium text-[#55504a]" title={signedInEmail}>{signedInEmail}</span>}</div>
