...
export function TopBar({ onOpenMenu, menuOpen, locale: _locale = "en", signedInName, signedInEmail }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#dfe5ee] bg-white/85 px-5 text-[#0b1220] backdrop-blur-xl lg:hidden">
      <div className="flex min-w-0 items-center gap-3"><Link href="/" aria-label="ORENDALIS Home" className="rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3457d5]"><OrendalisMark /></Link>{identityLabel && <span className="max-w-[42vw] truncate rounded-full bg-[#edf1f7] px-3 py-1 text-xs font-medium text-[#4f5d72]" title={identityLabel}>{identityLabel}</span>}</div>
