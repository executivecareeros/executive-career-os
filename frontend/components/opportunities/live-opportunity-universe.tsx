...
function CollectedOpportunityCard({ opportunity }: { opportunity: Opportunity }) {
  ...
  return <article className="rounded-2xl border border-white/10 bg-white/[.04] p-5 sm:p-6"><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-slate-900 text-xs font-semibold text-slate-300">{opportunity.companyInitials}</div><div><p className="text-sm text-blue-300">{opportunity.companyName}</p><h2 className="mt-1 text-lg font-semibold text-white">{opportunity.jobTitle}</h2><p className="mt-2 text-sm text-slate-500">{opportunity.location} · {opportunity.workArrangement}</p></div></div><p className="mt-4 line-clamp-3 text-sm leading-6 text-slate-400">{opportunity.summary ? toPlainText(opportunity.summary) : "The employer has published this role. Open the source to review the confirmed description."}</p><div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-500"><span>{opportunity.sources?.length ?? 1} verified source{(opportunity.sources?.length ?? 1) === 1 ? "" : "s"}</span><span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2.5 py-1 text-emerald-200">{freshness.status} · observed {freshness.ageHours}h ago</span></div><div className="mt-5 flex flex-wrap gap-3"><Link href={`/opportunities/${encodeURIComponent(opportunity.id)}`} className="inline-flex rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-slate-950 hover:bg-slate-200">Open Atlas intelligence</Link>{opportunity.sourceUrl && <a href={opportunity.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex rounded-xl border border-white/15 px-4 py-2.5 text-sm font-medium text-white hover:bg-white/5">Published role ↗</a>}</div></article>;
}
