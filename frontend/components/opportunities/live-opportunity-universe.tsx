...
export function LiveOpportunityUniverse({ opportunity, collected, collectionNotice, collectionMessage, imported, found, collectionAction }: { opportunity?: LiveOpportunityViewModel; collected: Opportunity[]; collectionNotice?: string; collectionMessage?: string; imported?: string; found?: string; collectionAction: (formData: FormData) => void | Promise<void> }) {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-10">
    ...
    <section className="mt-6 rounded-2xl border border-white/10 bg-white/[.04] p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="atlas-kicker">Live collection</p><h2 className="mt-2 font-semibold text-white">Add an employer&apos;s published opportunities</h2><p className="mt-2 max-w-2xl text-sm text-slate-400">Paste a public Greenhouse careers URL. Orendalis reads published roles only, attributes every source, and updates matching records without creating duplicates.</p></div><form action={collectionAction} className="flex w-full max-w-xl flex-col gap-2 sm:flex-row"><label className="sr-only" htmlFor="greenhouse-board">Company Greenhouse careers URL</label><input id="greenhouse-board" name="board" required placeholder="https://boards.greenhouse.io/company" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-slate-950 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:ring-2 focus:ring-blue-400"/><button className="rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-slate-950 hover:bg-slate-200">Refresh roles</button></form></div>{collectionNotice && <p className={`mt-4 rounded-xl border px-4 py-3 text-sm ${collectionNotice === "complete" ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-200" : "border-rose-400/20 bg-rose-400/10 text-rose-200"}`} role="status">{collectionNotice === "complete" ? `${found ?? "0"} published roles observed; ${imported ?? "0"} new or changed opportunities preserved.` : collectionMessage ?? "The careers board could not be refreshed."}</p>}</section>
...
function CollectedOpportunityCard({ opportunity }: { opportunity: Opportunity }) {
  ...
  return <article className="rounded-2xl border border-white/10 bg-white/[.04] p-5 sm:p-6"><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-slate-900 text-xs font-semibold text-slate-300">{opportunity.companyInitials}</div><div><p className="text-sm text-blue-300">{opportunity.companyName}</p><h2 className="mt-1 text-lg font-semibold text-white">{opportunity.jobTitle}</h2><p className="mt-2 text-sm text-slate-500">{opportunity.location} · {opportunity.workArrangement}</p></div></div><p className="mt-4 line-clamp-3 text-sm leading-6 text-slate-400">{opportunity.summary || "The employer has published this role. Open the source to review the confirmed description."}</p><div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-500"><span>Source: {opportunity.source}</span><span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2.5 py-1 text-emerald-200">{freshness.status} · observed {freshness.ageHours}h ago</span></div><div className="mt-5 flex flex-wrap gap-3">{opportunity.sourceUrl && <a href={opportunity.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-slate-950 hover:bg-slate-200">Review published role</a>}<span className="self-center text-xs text-slate-500">Atlas recommendation pending confirmed Blueprint fit</span></div></article>;
}
