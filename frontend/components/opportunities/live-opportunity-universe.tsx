...
import { classifyGeographicEligibility, founderGeographicProfile, opportunityConfidence, confidenceLabel, sortForExecutive } from "@/lib/opportunity-geography";
...
export function LiveOpportunityUniverse({ opportunity, collected, collectionNotice, collectionMessage, imported, found, collectionAction, linkedInAction, alertAction, linkedInNotice, verification, linkedInResetKey, initialQuery = "", locale = "en", cvComplete, savedRoles, newRoles }: Props) {
  ...
  const tr = locale === "tr";
  const collectedVisible = useMemo(() => sortForExecutive(collected.filter((item) => {
  ...
  }), founderGeographicProfile), [collected, company, industry, location, minimumSalary, query, view, workModel]);
...
function CollectedOpportunityCard({ opportunity, locale }: { opportunity: Opportunity; locale: Locale }) {
  ...
  const tr = locale === "tr";
  const eligibility = classifyGeographicEligibility(opportunity, founderGeographicProfile);
  const confidence = opportunityConfidence(opportunity, founderGeographicProfile);
