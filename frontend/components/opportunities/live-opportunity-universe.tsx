...
export function LiveOpportunityUniverse({ opportunity, collected, geographicProfile, careerContext, canConfirmFounderFixture, profileConfirmationAction, collectionNotice, collectionMessage, imported, found, collectionAction, initialQuery = "", cvComplete, savedRoles, newRoles }: Props) {
  ...
  const collectedVisible = useMemo(() => {
    if (view === "Recommended") return sortOpportunitiesForExecutive(collected, geographicProfile, careerContext).filter((item) => assessOpportunityConfidence(item, geographicProfile, careerContext).eligibility !== "Not Currently Eligible").slice(0, 12);
  ...
  }, [applied, careerContext, collected, eligibleOnly, geographicProfile, sort, view]);
  ...
  function submitSearch(event: FormEvent) {
    ...
    setValidation(""); setApplied(draft); setView("Search");
    if (draft.query.trim()) setRecent((current) => [draft.query.trim(), ...current.filter((item) => item !== draft.query.trim())].slice(0, 5));
  }
  const clearFilters = () => { const empty = emptyFilters(); setDraft(empty); setApplied(empty); setValidation(""); };
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-10">
    ...
    {view === "Search" && <form onSubmit={submitSearch} className="mt-6 rounded-2xl border border-[#e3e5e6] bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold text-[#17191c]">Search executive jobs</h2><p className="mt-1 text-sm text-[#6f757b]">Your selections are <strong>User Preferences</strong>. Atlas recommendations and automatic detections are shown separately and never overwrite them.</p></div><button type="button" onClick={clearFilters} className="text-sm font-semibold text-[#536f80]">Clear all</button></div>
    ...
    <div className="mt-6">{view === "Recommended" && opportunity && <LiveOpportunityCard opportunity={opportunity} />}{(view === "Search" || (view === "Recommended" && !opportunity)) && collectedVisible.length > 0 && <div className="grid gap-5 xl:grid-cols-2">{collectedVisible.map((item) => <CollectedOpportunityCard key={item.id} opportunity={item} geographicProfile={geographicProfile} careerContext={careerContext} />)}</div>}{view === "Recommended" && !opportunity && !collectedVisible.length && <EmptyState eyebrow="Atlas recommendations" title="No recommendations yet" description="Atlas will rank opportunities when confirmed experience and preferences provide enough evidence." action={<SecondaryButton href="/workspace">Review profile</SecondaryButton>} />}{view === "Search" && !collectedVisible.length && <EmptyState eyebrow="Search jobs" title={collected.length ? "No jobs match these filters" : "Your job search starts here"} description={collected.length ? "Try widening one or more filters." : "Opportunities will appear as verified employer sources are connected."} action={collected.length ? <SecondaryButton onClick={clearFilters}>Clear filters</SecondaryButton> : <SecondaryButton href="/import">Upload your CV</SecondaryButton>} />}</div>
  </div>;
}
...
function CollectedOpportunityCard({ opportunity, geographicProfile, careerContext }: { opportunity: Opportunity; geographicProfile: ExecutiveGeographicProfile; careerContext: ExecutiveCareerContext }) {
  ...
  const salary = opportunity.salaryMin !== undefined || opportunity.salaryMax !== undefined ? `${opportunity.salaryCurrency ?? "Currency unconfirmed"} ${opportunity.salaryMin?.toLocaleString("en-GB") ?? "?"}${opportunity.salaryMax !== undefined ? `–${opportunity.salaryMax.toLocaleString("en-GB")}` : ""}` : "Compensation not disclosed";
  return <article className="rounded-2xl border border-[#e3e5e6] bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#f1f3f3] text-xs font-semibold text-[#626970]">{opportunity.companyInitials}</div><div><p className="text-sm font-medium text-[#536f80]">{opportunity.companyName}</p><h2 className="mt-1 text-lg font-semibold text-[#17191c]">{opportunity.jobTitle}</h2><p className="mt-2 text-sm text-[#747b82]">{opportunity.location} · {opportunity.workArrangement}</p><p className="mt-1 text-sm font-medium text-[#30343a]">{salary} <span className="font-normal text-[#7a8086]">· {opportunity.salaryDisclosure ?? "Not disclosed"}</span></p><div className="mt-2 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-[#edf3f5] px-2 py-1 font-medium text-[#36576a]">{confidence.label} · {confidence.opportunityConfidence}%</span><span className="rounded-full bg-[#f5f6f6] px-2 py-1 text-[#626970]">{confidence.eligibility}</span></div><p className="mt-2 text-xs text-[#626970]">{confidence.explanation}</p></div></div><p className="mt-4 line-clamp-3 text-sm leading-6 text-[#626970]">{opportunity.summary ? toPlainText(opportunity.summary) : "Open the role to review the employer's published description."}</p><div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-[#7a8086]"><span>{opportunity.source}</span><span>Updated {freshness.ageHours}h ago</span><span>Industry: {opportunity.industry} ({opportunity.industryClassification?.source ?? "Source record"})</span></div><div className="mt-5 flex flex-wrap gap-3"><Link href={`/opportunities/${encodeURIComponent(opportunity.id)}`} className="inline-flex rounded-xl bg-[#17191c] px-4 py-2.5 text-sm font-medium text-white hover:bg-black">Review opportunity</Link>{opportunity.sourceUrl && <a href={opportunity.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex rounded-xl border border-[#d9dcde] px-4 py-2.5 text-sm font-medium text-[#30343a] hover:bg-[#f5f6f6]">Original listing ↗</a>}</div></article>;
}
