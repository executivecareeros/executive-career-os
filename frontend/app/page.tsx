...
export async function ExecutiveBriefing({ resolved }: { resolved: NonNullable<Awaited<ReturnType<typeof resolveAuthenticatedRepositoryContext>>> }) {
  ...
  if (!view) return <div className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-10"><PageHeader eyebrow="Home" title="Good morning." description="Search executive jobs now, or upload your CV so Atlas can begin preparing recommendations." actions={<PrimaryButton href="/import">Upload your CV</PrimaryButton>}/><form action="/opportunities" className="mt-8 flex max-w-3xl flex-col gap-3 rounded-2xl border border-[#e3e5e6] bg-white p-4 shadow-sm sm:flex-row"><label className="sr-only" htmlFor="home-job-search">Search executive jobs</label><input id="home-job-search" name="q" type="search" placeholder="Search by role, company, or location" className="min-h-12 min-w-0 flex-1 rounded-xl border border-[#d9dcde] px-4 text-[#17191c] placeholder:text-[#9aa0a5]"/><button className="rounded-xl bg-[#17191c] px-6 py-3 text-sm font-semibold text-white">Search jobs</button></form><div className="mt-8"><DashboardSection title="Atlas recommendations" description="Atlas will keep this quiet until there is enough context to be useful." emptyTitle="No recommendations yet" emptyDescription="Upload your CV or save a few roles. You can keep searching in the meantime." action={<SecondaryButton href="/opportunities">Browse jobs</SecondaryButton>}/></div></div>;
