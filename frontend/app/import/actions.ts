...
export async function confirmCvHistory(formData: FormData) {
  ...
  drafts = drafts.map(draft => ({ ...draft, organizationName: String(draft.organizationName ?? "").trim().slice(0, 200), roleTitle: String(draft.roleTitle ?? "").trim().slice(0, 200), startDate: databaseDate(String(draft.startDate ?? "")), endDate: databaseDate(String(draft.endDate ?? "")), isCurrent: Boolean(draft.isCurrent), companyDescription: String(draft.companyDescription ?? "").trim().slice(0, 3000), roleDescription: String(draft.roleDescription ?? "").trim().slice(0, 5000), responsibilities: Array.isArray(draft.responsibilities) ? draft.responsibilities.map(String).slice(0, 30) : [], achievements: Array.isArray(draft.achievements) ? draft.achievements.map(String).slice(0, 30) : [], location: String(draft.location ?? "").trim().slice(0, 200), employmentType: String(draft.employmentType ?? "").trim().slice(0, 100), leadershipScope: String(draft.leadershipScope ?? "").trim().slice(0, 1500), teamSize: String(draft.teamSize ?? "").trim().slice(0, 100), geographicResponsibility: String(draft.geographicResponsibility ?? "").trim().slice(0, 500), revenueScope: String(draft.revenueScope ?? "").trim().slice(0, 500), industries: Array.isArray(draft.industries) ? draft.industries.map(String).slice(0, 20) : [], technologies: Array.isArray(draft.technologies) ? draft.technologies.map(String).slice(0, 30) : [] }));
