...
export default function DiscoveryPage() {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-10">
    ...
    <SectionCard className="mt-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="atlas-kicker">Ingestion foundation</p><h2 className="mt-3 text-xl font-semibold">Built for attributable, refreshable opportunity sources</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Approved providers enter through one validation and normalization pipeline. Every record retains its source, freshness, lifecycle, duplicate history, and failure outcome before Atlas can evaluate it.</p></div><StatusBadge tone="success">Pipeline ready</StatusBadge></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Collect","Provider health and limits"],["Normalize","One Opportunity object"],["Reconcile","Cross-source duplicates"],["Observe","Freshness and failures"]].map(([title,detail])=><div key={title} className="rounded-xl border border-white/10 bg-slate-950/40 p-4"><p className="font-medium text-white">{title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{detail}</p></div>)}</div><p className="mt-4 text-xs text-slate-500">No live provider is activated. Provider credentials, legal review, and production scheduling remain separate approval gates.</p></SectionCard>
