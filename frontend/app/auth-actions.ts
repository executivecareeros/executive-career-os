...
import { acceptRememberedInvitation, inspectInvitation, rememberInvitation } from "@/lib/beta/invitations";

function value(form: FormData, key: string) { return String(form.get(key) ?? "").trim(); }
async function origin() { const h = await headers(); return h.get("origin") ?? "http://localhost:3000"; }

export async function loginAction(form: FormData) {
  try { const session = await supabaseAuth.signIn(value(form,"email"),value(form,"password")); await acceptRememberedInvitation(session.access_token); await storeSession(session,form.get("remember")==="on"); } catch (error) { redirect(`/login?error=${encodeURIComponent(error instanceof Error?error.message:"Unable to sign in")}`); }
  redirect("/welcome");
}
export async function registerAction(form: FormData) {
  const password=value(form,"password"),email=value(form,"email"),inviteToken=value(form,"inviteToken"); if(password.length<8) redirect(`/register?invite=${encodeURIComponent(inviteToken)}&error=Password%20must%20contain%20at%20least%208%20characters`);
  let session;try { const invitation=await inspectInvitation(email,inviteToken);if(invitation.invitation_status!=="Pending")throw new Error(`Invitation is ${invitation.invitation_status.toLowerCase()}.`);await rememberInvitation(inviteToken);session=await supabaseAuth.signUp(email,password,`${await origin()}/login?verified=1`);if(session.access_token){await acceptRememberedInvitation(session.access_token);await storeSession(session,true);}} catch(error){redirect(`/register?invite=${encodeURIComponent(inviteToken)}&error=${encodeURIComponent(error instanceof Error?error.message:"Unable to register")}`);}if(!session?.access_token)redirect("/login?verification=sent");redirect("/welcome");
}
