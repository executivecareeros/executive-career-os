...
export async function saveGuidanceAnswersAction(formData: FormData) {
  ...
  const selected = formData.getAll("selectedQuestion").map(String).filter(id => /^[a-zA-Z0-9:_-]{3,200}$/.test(id)).slice(0, 20);
  ...
  const rows = selected.map(questionId => {
    ...
    const answer = String(formData.get(`answer:${questionId}`) ?? "").trim().slice(0, 2000);
  ...
  redirect(`/assistant?guidance=saved&count=${rows.length}`);
}
