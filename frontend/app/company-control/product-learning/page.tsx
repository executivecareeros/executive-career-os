...
function Distribution({ title, values }: { title: string; values: Array<{ name: string; executives?: number; sessions?: number; events?: number }> }) {
  return <SectionCard><h2 className="text-lg font-semibold text-white">{title}</h2><div className="mt-4 space-y-3">{values.length ? values.map((item) => <div key={item.name} className="flex items-center justify-between gap-4 border-b border-white/5 pb-3 text-sm"><span className="text-slate-300">{labels[item.name] ?? item.name}</span><span className="font-semibold text-white">{item.executives ?? item.sessions ?? item.events ?? 0}</span></div>) : <p className="text-sm text-slate-500">No confirmed aggregate yet.</p>}</div></SectionCard>;
}
...
export default async function ProductLearningPage() {
  ...
  const headline = [
    ["Executives", data.executives], ["Sessions", data.sessions], ["Returning executives", data.returningExecutives], ["Average session", `${Math.floor(data.averageSessionSeconds / 60)}m ${data.averageSessionSeconds % 60}s`],
  ] as const;
  return <div className="mx-auto max-w-[1400px] px-4 py-6 sm:px-6 lg:px-8">
    ...
    <section className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{headline.map(([label,value]) => <article key={label} className="rounded-2xl border border-white/10 bg-slate-950/60 p-5"><p className="text-xs uppercase tracking-[.16em] text-slate-500">{label}</p><p className="mt-3 text-3xl font-semibold text-white">{value}</p></article>)}</section>
