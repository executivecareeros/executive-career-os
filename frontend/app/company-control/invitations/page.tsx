...
export const metadata:Metadata={title:"Invitation Management",description:"Founder-only private beta invitation management."};
export const dynamic="force-dynamic";
export default async function InvitationManagementPage(){const access=await resolveFounderAccess();if(!access)notFound();const invitations=await new FounderInvitationService(createServerSupabaseClient(access.accessToken),access.context.workspace!.workspaceId).list();return <main className="mx-auto max-w-[1400px] px-4 py-6 sm:px-6 lg:px-10"><Link href="/company-control" className="text-sm text-blue-300 hover:text-blue-200">← Company Control</Link><div className="mt-5"><PageHeader eyebrow="Founder-only access" title="Invitation Management" description="Control private beta access without direct database administration."/></div><div className="mt-7"><InvitationManagement initialInvitations={invitations}/></div></main>;}
