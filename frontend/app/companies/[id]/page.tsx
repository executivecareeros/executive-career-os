...
import { buildCanonicalEmployerIntelligence } from "@/lib/employer-intelligence";

type OpportunityRow = { domain_id: string; company_id?: string; payload: Record<string, unknown>; updated_at?: string };
type CompanyRow = { id: string; name: string; country?: string; industry?: string; official_domain?: string; careers_url?: string; ats_provider?: string; identity_confidence: number; last_observed_at?: string; last_verified_at?: string; payload?: Record<string, unknown> };
...
export default async function CompanyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-10 sm:px-6 lg:px-10">
    ...
    <div className="mt-4 flex flex-col justify-between gap-5 md:flex-row md:items-start"><div><h1 className="text-4xl font-semibold tracking-[-.04em] text-[#17191c]">{companyName}</h1><p className="mt-3 max-w-2xl text-[#626970]">{intelligence.overview}</p></div><span className="w-fit rounded-full bg-[#f3eee7] px-4 py-2 text-sm font-medium text-[#695238]">{intelligence.hiringActivity} hiring activity</span></div>
    <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Fact label="Industry" value={intelligence.industry}/><Fact label="Headquarters" value={intelligence.headquarters}/><Fact label="Operating and hiring countries" value={intelligence.operatingCountries.join(", ") || "Unknown"}/><Fact label="Company size" value={intelligence.companySize}/><Fact label="Active opportunities" value={String(intelligence.activeOpportunities)}/><Fact label="Executive opportunities" value={String(intelligence.executiveOpportunities)}/><Fact label="Commercial opportunities" value={String(intelligence.commercialOpportunities)}/><Fact label="Hiring locations" value={String(intelligence.hiringLocations.length)}/><Fact label="Source freshness" value={intelligence.freshness}/><Fact label="Evidence confidence" value={`${intelligence.confidence}%`}/><Fact label="Provider" value={sources.join(", ") || "Unknown"}/><Fact label="Intelligence version" value={intelligence.fingerprint.split(":")[0]}/></section>
    <section className="mt-8 grid gap-6 lg:grid-cols-2"><div className="rounded-2xl border border-[#e3e5e6] bg-white p-6"><h2 className="text-xl font-semibold">Markets and hiring footprint</h2><p className="mt-3 text-sm leading-6 text-[#626970]">{intelligence.markets.length ? intelligence.markets.join(", ") : "Markets are not confirmed."}</p><div className="mt-4 flex flex-wrap gap-2">{intelligence.hiringLocations.slice(0, 12).map((location) => <span key={location} className="rounded-full bg-[#f3f4f4] px-3 py-1.5 text-xs">{location}</span>)}</div></div><div className="rounded-2xl border border-[#e3e5e6] bg-white p-6"><h2 className="text-xl font-semibold">Products and services</h2><p className="mt-3 text-sm leading-6 text-[#626970]">{intelligence.productsAndServices.length ? intelligence.productsAndServices.join(", ") : "No authoritative product or service evidence is available yet. Orendalis will not infer it from job descriptions."}</p></div></section>
    <section className="mt-8 rounded-2xl border border-[#e3e5e6] bg-white p-6"><h2 className="text-xl font-semibold">Evidence</h2><div className="mt-4 divide-y divide-[#eceeef]">{intelligence.evidence.map((item) => <div key={`${item.label}-${item.value}`} className="grid gap-2 py-4 first:pt-0 sm:grid-cols-[10rem_1fr_auto]"><p className="text-sm font-semibold">{item.label}</p><div><p className="text-sm text-[#30343a]">{item.value}</p><p className="mt-1 text-xs text-[#7a8086]">{item.source}</p></div><span className="text-xs font-semibold text-[#55705d]">{item.confidence}%</span></div>)}</div></section>
