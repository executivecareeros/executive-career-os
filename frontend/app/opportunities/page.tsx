...
export default async function OpportunitiesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (process.env.NEXT_PUBLIC_DATA_ACCESS_MODE === "supabase") {
    ...
    if (opportunity || collected.length) return <LiveOpportunityUniverse locale={locale} opportunity={opportunity} collected={collected} initialQuery={typeof query.q === "string" ? query.q : ""} collectionNotice={typeof query.collection === "string" ? query.collection : undefined} collectionMessage={typeof query.message === "string" ? query.message : undefined} imported={typeof query.imported === "string" ? query.imported : undefined} found={typeof query.found === "string" ? query.found : undefined} linkedInNotice={typeof query.linkedin === "string" ? query.linkedin : undefined} verification={typeof query.verification === "string" ? query.verification : undefined} linkedInResetKey={typeof query.completedAt === "string" ? query.completedAt : undefined} collectionAction={refreshOpportunityBoard} linkedInAction={importLinkedInOpportunity} alertAction={importLinkedInJobAlert} />;
