...
import { refreshGreenhouseBoard } from "./actions";
...
export default async function OpportunitiesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  if (process.env.NEXT_PUBLIC_DATA_ACCESS_MODE === "supabase") {
    ...
    if (opportunity || collected.length) return <LiveOpportunityUniverse opportunity={opportunity} collected={collected} collectionNotice={typeof query.collection === "string" ? query.collection : undefined} collectionMessage={typeof query.message === "string" ? query.message : undefined} imported={typeof query.imported === "string" ? query.imported : undefined} found={typeof query.found === "string" ? query.found : undefined} collectionAction={refreshGreenhouseBoard} />;
    return <OpportunityUniverseEmpty collectionAction={refreshGreenhouseBoard} />;
  }
