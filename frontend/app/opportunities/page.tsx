...
// The list view deliberately excludes long job-description fields. Loading a
// thousand complete provider payloads made the executive wait tens of seconds
// even though only twelve cards are visible. Opportunity detail still loads the
// authoritative full payload by canonical id.
const opportunityListSelect = [
  ...
  "industry:payload->>industry", "industryClassification:payload->industryClassification", "companySize:payload->>companySize", "source:payload->>source",
  "sourceUrl:payload->>sourceUrl", "sources:payload->sources", "verificationStatus:payload->>verificationStatus", "lastObservedAt:payload->>lastObservedAt",
  "freshness:payload->freshness", "publishedAt:payload->>publishedAt", "discoveredAt:payload->>discoveredAt", "salaryMin:payload->>salaryMin",
...
export default async function OpportunitiesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  if (process.env.NEXT_PUBLIC_DATA_ACCESS_MODE === "supabase") {
    ...
    try {
      ...
      const rows = await createServerSupabaseClient(resolved.accessToken).request<OpportunityListRow[]>(`opportunities?select=${opportunityListSelect}&workspace_id=eq.${resolved.context.workspace!.workspaceId}&archived_at=is.null&order=updated_at.desc&limit=1000`);
