...
import { parseStructuredResume } from "./structured-resume";
...
export interface HistoryDocumentExtraction {
  ...
  certifications: Array<{ name: string; issuer?: string; year?: string; evidence: string; confidence: "High" | "Medium" | "Low" | "Unknown" }>;
  rawText: string;
}
...
export async function extractHistoryDocument(file:File):Promise<HistoryDocumentExtraction>{
  ...
  const structured=parseStructuredResume(text);
