...
export async function loadExecutiveProfileState(client:SupabaseDataClient,workspaceId:string,executiveId:string):Promise<ExecutiveProfileState>{
  ...
  if(experiences.error)throw new Error("EXECUTIVE_PROFILE_STATE_UNAVAILABLE");
  return deriveExecutiveProfileState(sessions.error?[]:sessions.data??[],experiences.data??[]);
}
