...
export async function resolveAuthenticatedRepositoryContext(options?:{requiredRole?:ContextRow["role"]}): Promise<{ context: RepositoryContext; accessToken: string } | undefined> {
  ...
  const requiredRole=options?.requiredRole?`&role=eq.${encodeURIComponent(options.requiredRole)}`:"";
  const response = await client.request<ContextRow[]>(`workspace_memberships?select=id,role,workspace_id,executive_identity_id&status=eq.Active&archived_at=is.null${requiredRole}&limit=1`);
