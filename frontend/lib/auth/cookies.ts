...
export const REMEMBER_COOKIE = "ecos-remember-session";
...
export async function storeSession(session: AuthSession, remember: boolean) {
  ...
  jar.set(ACCESS_COOKIE, session.access_token, { ...common, ...(remember ? { maxAge: session.expires_in } : {}) });
  jar.set(REFRESH_COOKIE, session.refresh_token, { ...common, ...persistent });
  jar.set(REMEMBER_COOKIE, remember ? "1" : "0", { ...common, ...persistent });
}
export async function clearSession() { const jar = await cookies(); jar.delete(ACCESS_COOKIE); jar.delete(REFRESH_COOKIE); jar.delete(REMEMBER_COOKIE); }
