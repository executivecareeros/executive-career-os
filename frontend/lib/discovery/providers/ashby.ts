...
export class AshbyOpportunityProvider implements OpportunityProvider {
  ...
  async collect(request: ProviderCollectionRequest) {
    ...
    return { providerId: this.id, collectedAt, jobs, sourceRevision: `${response.apiVersion}:${listed.length}`, completeSnapshot: listed.length <= request.maximumResults, snapshotScopeKeys: [`ashby:${this.board}`] } as const;
  }
