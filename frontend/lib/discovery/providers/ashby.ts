...
export class AshbyOpportunityProvider implements OpportunityProvider {
  ...
  constructor(readonly board: string, private readonly fetcher: typeof fetch = fetch) {}
  ...
  async collect(request: ProviderCollectionRequest) {
    ...
    const jobs: DiscoveryJob[] = response.jobs.filter((job) => job.isListed !== false).slice(0, request.maximumResults).map((job) => { const salary = job.compensation?.summaryComponents?.find((component) => component.compensationType === "Salary"); return { sourceId: `${this.board}-${job.jobUrl.split("/").filter(Boolean).at(-1)}`, source: this.id, title: job.title, company: { sourceId: this.board, name: this.board, country: job.address?.postalAddress?.addressCountry, website: `https://jobs.ashbyhq.com/${this.board}` }, location: job.location, country: job.address?.postalAddress?.addressCountry, description: job.descriptionPlain, originalUrl: job.jobUrl, publishedAt: job.publishedAt, discoveredAt: collectedAt, employmentType: employment(job.employmentType), salary: salary ? { minimum: salary.minValue ?? undefined, maximum: salary.maxValue ?? undefined, currency: salary.currencyCode } : undefined, rawMetadata: { board: this.board, apiVersion: response.apiVersion, department: job.department, team: job.team, workArrangement: arrangement(job.workplaceType) } }; });
    return { providerId: this.id, collectedAt, jobs, sourceRevision: `${response.apiVersion}:${jobs.length}` } as const;
  }
