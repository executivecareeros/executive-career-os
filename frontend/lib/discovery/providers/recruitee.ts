...
export class RecruiteeOpportunityProvider implements OpportunityProvider {
  ...
  async collect(request: ProviderCollectionRequest) { const collectedAt = new Date().toISOString(); const response = await this.offers(); const offers = (response.offers ?? []).filter((offer) => offer.title && offer.careers_url).slice(0, request.maximumResults); const jobs: DiscoveryJob[] = offers.map((offer) => ({ sourceId: `${this.company}-${offer.id}`, source: this.id, title: offer.title, company: { sourceId: this.company, canonicalKey: `recruitee:${this.company}`, name: offer.company_name ?? this.company, country: offer.country ?? offer.country_code }, location: offer.location ?? offer.city, country: offer.country ?? offer.country_code, description: plainText(`${offer.description ?? ""} ${offer.requirements ?? ""}`), originalUrl: offer.careers_url, publishedAt: offer.updated_at ?? offer.published_at, discoveredAt: collectedAt, employmentType: employment(offer.employment_type_code), salary: offer.salary ? { minimum: offer.salary.min ?? undefined, maximum: offer.salary.max ?? undefined, currency: offer.salary.currency ?? undefined } : undefined, rawMetadata: { company: this.company, department: offer.department, workArrangement: offer.remote ? "Remote" : offer.hybrid ? "Hybrid" : undefined } })); return { providerId: this.id, collectedAt, jobs, sourceRevision: `${this.company}:${jobs.length}` } as const; }
