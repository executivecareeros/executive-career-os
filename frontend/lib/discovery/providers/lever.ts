...
export class LeverOpportunityProvider implements OpportunityProvider {
  ...
  async collect(request: ProviderCollectionRequest) {
    ...
    return { providerId: this.id, collectedAt, jobs, sourceRevision: `${this.region}:${this.site}:${jobs.length}`, completeSnapshot, snapshotScopeKeys: [`lever:${this.region}:${this.site}`] } as const;
  }
