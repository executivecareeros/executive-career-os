...
const onboarding = psql(`begin;set local role authenticated;select set_config('request.jwt.claim.sub','90000000-0000-4000-8000-000000000001',true);
select public.provision_personal_workspace('{"preferredName":"Runtime Executive","currentRole":"Chief Test Officer","country":"Example Country","preferredLanguage":"English","timezone":"UTC","careerAmbition":"Build durable professional judgment","atlasPromiseAccepted":true}'::jsonb);
select public.provision_personal_workspace('{"preferredName":"Runtime Executive","currentRole":"Chief Test Officer","country":"Example Country","preferredLanguage":"English","timezone":"UTC","careerAmbition":"Build durable professional judgment","atlasPromiseAccepted":true}'::jsonb);
reset role;select count(*) from public.executive_identities where auth_user_id='90000000-0000-4000-8000-000000000001';select count(*) from public.workspaces where name='Runtime Executive''s Career Memory';select count(*) from public.workspace_memberships where executive_identity_id=(select id from public.executive_identities where auth_user_id='90000000-0000-4000-8000-000000000001');select count(*) from public.executive_blueprints where workspace_id=(select id from public.workspaces where name='Runtime Executive''s Career Memory');select count(*) from public.atlas_decision_snapshots where workspace_id=(select id from public.workspaces where name='Runtime Executive''s Career Memory');rollback;`).trim().split("\n");
const onboardingCounts = onboarding.filter((line) => /^\d+$/.test(line)).slice(-5).map(Number);
...
pass("Authenticated onboarding", "identity, workspace, owner membership, empty blueprint, and Atlas context created once");
