...
const checks = {
  ...
  opportunity_list_uses_bounded_indexed_projection: opportunityIndex.includes("loadNetworkOpportunities(EXECUTIVE_OPPORTUNITY_CANDIDATE_LIMIT)") && opportunityNetwork.includes("EXECUTIVE_OPPORTUNITY_CANDIDATE_LIMIT = 1_000") && opportunityNetwork.includes("select=id,domain_id,company_id,version,payload,updated_at") && opportunityNetwork.includes("status=in.(Discovered,Evaluating,Qualified,Ready%20to%20Apply,Applied,Interview)") && opportunityNetwork.includes("Math.min(EXECUTIVE_OPPORTUNITY_CANDIDATE_LIMIT, limit)") && !opportunityIndex.includes("select=domain_id,payload&workspace_id"),
};
