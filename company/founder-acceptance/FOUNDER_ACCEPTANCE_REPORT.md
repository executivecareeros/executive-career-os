...
| Email verification | Passed locally after correction | Verification succeeds without exposing tokens. | Supabase local mail capture received the fictional confirmation message within seconds. Its single-use link returned to onboarding, created a verified session, and accepted the invitation. Replay and invalid redirects failed safely. | FA-003 is resolved for product correctness. Internet delivery and production sender authentication remain explicitly unaccepted external gates. |
...
FA-003 was corrected by enabling confirmation in the local provider, adding a single-use callback, enforcing provider confirmation in the server session and PostgreSQL invitation/workspace functions, and adding a controlled resend surface. A configured application origin corrected the callback cookie-scope defect found during retest. The fictional journey passed through local delivery, verification, onboarding, beta workflow, logout, login, persistence, and founder-side Accepted status.
