...
|---|---|---|---|---|---|---|---|---|
| Register | `/register` | `registerAction`, `supabaseAuth.signUp` | Supabase Auth | Provider-backed in Supabase mode | Locally accepted | Production Supabase, redirect allowlist, invitation control, rate limits | External delivery not accepted | Repeat with isolated staging provider |
| Verify email | `/verify-email`, `/auth/confirm` | Supabase Auth verification and strict invitation revalidation | Supabase Auth plus invitation audit | Provider-backed | Locally accepted | Mail delivery, approved application origin, callback allowlist | Local capture proves product flow, not Internet delivery | Test external delivery on staging with a fictional identity |
...
| Email verification acceptance | Pass within local boundary | A fictional confirmation message arrived in Supabase Mailpit within seconds; callback, session, invitation acceptance, onboarding, workflow access, replay denial, resend rate limit, logout/login return, and Accepted status passed. External delivery remains unaccepted. |
